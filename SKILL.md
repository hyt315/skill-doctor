---
name: skill-doctor
description: 审查与迭代任意 AI Agent 技能目录（SKILL.md/scripts/references），支持「审查体检」与「迭代进化」双模式。通过四大架构形态适配、50+ 项静态规则、动态实跑、负向用例与 40+ 条坑库对照找出流程走不通、口径矛盾、静默失效与安全泄露；结合 SEI 技能进化指数、通用五阶段进化范式、测试脚手架注入与演进方案生成，驱动技能持续迭代升维。当用户要求审查/体检/优化某个技能、提出技能改进方案、技能流程跑不通、或技能改动后需要回归验证时使用。
---
...
4. **40+ 条实战坑库对照**——从数百次真实重构中沉淀的最佳实践（详见 `references/坑库.md`）；
...
- **审查与修改动作解耦**：报告归报告，修改是另一个明确授权的动作；
- **每条缺陷必附证据**：注明文件行号、命令输出或复现步骤，不写“感觉有问题”；
- **只读铁律**：体检和演进分析过程严禁对目标代码和系统产生未经授权的破坏性副作用；
- **工程卫生铁律**：测试与审计副产物（如 `audit-report.txt`）在发版前必须物理清除；
